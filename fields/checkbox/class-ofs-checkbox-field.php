...
class OFS_Checkbox_Field extends OFS_Field {
	...
	public function __construct() {
		$this->name = _x('Checkbox', 'condition field name', 'ofs');
	}
			...
			<td>
				...
				if(!empty($field['config']['items'])) {
					foreach ($field['config']['items'] as $item) {
						?>
						<label><input type="checkbox" name="<?php echo esc_attr($field['field_id']); ?>[]" value="<?=esc_attr($item)?>" <?php checked( in_array($item, $filtered), true, true ); ?>> <?=esc_html($item)?></label>
						<?php
					}
				}
				echo '</div>';
