...
class OFS_Select_Field extends OFS_Field {
	...
	public function __construct() {
		$this->name = _x('Select', 'condition field name', 'ofs');
	}
				...
				<div class="ofsfc-<?=esc_attr($this->type)?>">
					...
					if(!empty($field['config']['items'])) {
						foreach ($field['config']['items'] as $item) {
							?>
							<option value="<?=esc_attr($item)?>" <?php selected($filtered, $item, true); ?>><?=esc_html($item)?></option>
							<?php
						}
					}
					?>
